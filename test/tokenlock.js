...
describe("TokenLock", () => {
  ...
  before(async () => {
    ...
    unlockBegin = new Date(config.TEAM_UNLOCK_BEGIN).getTime() / 1000;
    unlockCliff = new Date(config.TEAM_UNLOCK_CLIFF).getTime() / 1000;
    unlockEnd = new Date(config.TEAM_UNLOCK_END).getTime() / 1000;
    await deployments.fixture(["PUBToken", "TokenLockTeam"]);
    token = await ethers.getContract("PUBToken");
    tokenLock = await ethers.getContract("TokenLockTeam");
  ...
  it("should allow a proportional amount to be claimed after the cliff", async () => {
    ...
    const unlockAmount = lockAmount
      .mul(unlockCliff - unlockBegin)
      .div(unlockEnd - unlockBegin);
    expect(await tokenLock.claimableBalance(account2.address)).to.equal(
      unlockAmount
    );
    const balanceBefore = await token.balanceOf(account2.address);
    await expect(
      tokenLock.connect(account2).claim(account2.address, unlockAmount)
    )
      .to.emit(tokenLock, "Claimed")
      .withArgs(account2.address, account2.address, unlockAmount);
    expect(await token.balanceOf(account2.address)).to.equal(
      balanceBefore.add(unlockAmount)
    );
  });
  ...
  it("should allow all tokens to be claimed after the end", async () => {
    ...
    const balanceBefore = await token.balanceOf(account2.address);
    await expect(
      tokenLock.connect(account2).claim(account2.address, lockAmount)
    )
      .to.emit(tokenLock, "Claimed")
      .withArgs(account2.address, account2.address, lockAmount);
    expect(await token.balanceOf(account2.address)).to.equal(
      balanceBefore.add(lockAmount)
    );
  });
  ...
  it("should not allow new locks after the end", async () => {
    await setNextBlockTimestamp(unlockEnd);
    await expect(
      tokenLock.lock(account2.address, lockAmount)
